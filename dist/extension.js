...
// src/extension.ts
var SERVER_URL = "http://192.168.1.13:8001";
function activate(context) {
  console.log("AI Code Assistant ACTIVATED!");
  console.log("AI Code Assistant is now active!");
  const fixCode = vscode.commands.registerCommand("ai-code-assistant.fixCode", async () => {
    ...
    const instructions = await vscode.window.showInputBox({
      prompt: 'What would you like to fix? (e.g., "fix syntax errors", "add error handling")',
      placeHolder: "Describe the fix needed..."
    });
    vscode.window.showInformationMessage("\u{1F916} Sending to AI server...");
    try {
      let promptText;
      if (instructions) {
        promptText = `Fix this code: ${instructions}

Code:
${selectedText}`;
      } else {
        promptText = `Fix any issues in this code and explain what you changed:

${selectedText}`;
      }
      const response = await axios_default.post(`${SERVER_URL}/process`, {
        prompt: promptText,
        context: {
      ...
      const fixedCode = response.data.result;
      const edit = new vscode.WorkspaceEdit();
      edit.replace(editor.document.uri, selection, fixedCode);
  ...
  const generateCode = vscode.commands.registerCommand("ai-code-assistant.generateCode", async () => {
    const prompt = await vscode.window.showInputBox({
      prompt: "Describe the code you want to generate",
      placeHolder: 'e.g., "Write a function that fetches data from an API"'
    });
    if (!prompt) {
      return;
    }
    vscode.window.showInformationMessage("\u{1F916} Generating code...");
    try {
      const response = await axios_default.post(`${SERVER_URL}/process`, {
        prompt: `Generate code for: ${prompt}. Return ONLY the code, no explanations.`,
        task_type: "code"
      });
  ...
  const explainCode = vscode.commands.registerCommand("ai-code-assistant.explainCode", async () => {
    const editor = vscode.window.activeTextEditor;
    if (!editor) {
      return;
    }
    ...
    vscode.window.showInformationMessage("\u{1F916} Analyzing code...");
    try {
      const response = await axios_default.post(`${SERVER_URL}/process`, {
...
${selectedText}`,
      ...
      const explanation = response.data.result;
      ...
      panel.webview.html = `<!DOCTYPE html>
            ...
            <body style="padding: 20px; font-family: var(--vscode-font-family);">
                ...
                <div style="white-space: pre-wrap;">${escapedExplanation}</div>
                <hr>
  ...
  const generateTest = vscode.commands.registerCommand("ai-code-assistant.generateTest", async () => {
    const editor = vscode.window.activeTextEditor;
    if (!editor) {
      return;
    }
    ...
    vscode.window.showInformationMessage("\u{1F916} Generating Playwright tests...");
    try {
      const response = await axios_default.post(`${SERVER_URL}/process`, {
        prompt: `Generate Playwright test cases for this code:

${selectedText}`,
        task_type: "code"
      });
      const testCode = response.data.result;
      const fileName = editor.document.fileName.replace(/\.(js|ts|jsx|tsx)$/, ".spec.ts");
      const uri = vscode.Uri.file(fileName);
      await vscode.workspace.fs.writeFile(uri, Buffer.from(testCode));
      vscode.window.showInformationMessage(`\u2705 Tests saved to ${fileName}`);
  ...
  context.subscriptions.push(fixCode, generateCode, explainCode, generateTest);
}
function deactivate() {
}
